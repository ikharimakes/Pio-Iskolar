    ...
    if(isset($_POST['generate'])){
        ...
        if($report == 'status') {
            ...
            $status = "SELECT 
                    ...
                    FROM scholar WHERE batch_num = '$batch'";
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    <p> This report provides an overview of the current status of scholars under the Dr. Pio Valenzuela Scholarship Program for the school year <strong>'.$row['acad_year'].'</strong>. As of <strong>'.$date.'</strong>, there are a total of <strong>'.$row['scholar_count'].'</strong> scholars enrolled in the program for Batch Number <strong>'.$batch.'</strong>. The table below presents the current status of scholars under the Dr. Pio Valenzuela Scholarship Program, along with the total number of scholars based on their status: </p>
                    <br>
                    ...
                    $insertReport = $conn->prepare("INSERT INTO reports (batch_no, report_type, creation_date, acad_year, content) VALUES (?, ?, ?, ?, ?)");
                    $reportType = 'status';
                    $insertReport->bind_param("sssss", $batch, $reportType, $date, $row['acad_year'], $content);
            ...
            $profile = "SELECT 
                            (SELECT acad_year FROM batch_year WHERE batch_no = '$batch') AS acad_year,
                            (SELECT COUNT(*) FROM scholar WHERE batch_num = '$batch') AS scholar_count,
                        ...
                        FROM submission LEFT JOIN scholar ON submission.scholar_id = scholar.scholar_id WHERE scholar.batch_num = '$batch'";
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    <p> This report provides a comprehensive overview of the profile and current requirement status of scholars under the Dr. Pio Valenzuela Scholarship Program for the <strong>".$sem."</strong> Semester of S.Y. <strong>".$row['acad_year']."</strong>. As of <strong>".$date."</strong>, there are a total of <strong>".$row['scholar_count']."</strong> scholars enrolled in the program for Batch Number <strong>".$batch."</strong>. The table below presents the profile of scholars and the current status of their requirements, along with the total number of scholars who have completed their requirements, and the number of scholars with missing requirements. This report is crucial for monitoring the progress of scholars and ensuring that they meet the program's criteria and obligation. </p>
                    <br>
